...
.addStringOption(
    ...
    //         .setDescription(`The winner of the invite contest is: ${member_id}`)
    ...
    if (set_item == "WL" || set_item == "Wl" || set_item == "wl" || set_item == "wL")
    {
        ...
        for(var i = 0; i < (globalvar.wl_count+1); i++ )
        {
            ...
            if((((Date.parse(traverse.timer) < Date.now())) && (traverse.final_winner.length == 0)))
            {
                ...
                .addField("Cost for the ticket:", `${emoji_is}**\`${traverse.raffle.set_cost} $SKNK\`**`, true)
                .setColor("GREEN")
...
module.exports.button = async (bot, interaction, user, action, another_user) => {
    ...
    if ((globalvar.timer - Date.now()) < 0) 
    {
        ...
        var CongratsWinner = bot.channels.cache.find(channel => channel.id === `1002606738614259732`);
        //interaction.channel
        ...
        thread.send({
            content: `__Winners__: \n${members} \n__Team members to follow up on the prize__: \n<@328719626525736971> <@795174532939644968> <@303604936522989578>`
        });
        thread.send({
                    content: `${tagwins}`
        }).then(message => message.delete()); 
        
        if(globalvar.entries.length > 0)
        {
            var check = []
            console.log(globalvar.entries)
            for (item in globalvar.entries){
                if(!check.includes(globalvar.entries[item])){
                    check.push(globalvar.entries[item])
                }
            }
