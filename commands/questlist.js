...
module.exports.run = async (bot, Interaction, options) =>
{
    ...
    if(Interaction.channel.id != "1002622934592016394") return Interaction.editReply({content: `You can only use this command in ${CongratsWinner}`})
    ...
    queslist.addFields({name: "• Quest 7:", value: `${me_emoji}`})
    ...
    for(var j = 0; j < (all_quests.length);j++)
    {
        ...
        queslist.addFields({name: `• Quest ${8+j}:`, value: `${me_emoji}`})
    }

    var completed = 7 + all_quests.length
