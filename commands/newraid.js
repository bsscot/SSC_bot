...
module.exports.run = async (bot, Interaction, options) =>
{
    ...
    var quest_ch = bot.channels.cache.find(channel => channel.id === `1006252055389802586`);
    if(Interaction.channel.id != "1006252055389802586") return Interaction.editReply({content: `You can only use this command in ${quest_ch}`})
    
    let guild = Interaction.guild
    const emoji_is = await Interaction.guild.emojis.cache.find(emoji => emoji.name === 'BREWTOKEN')
    const emoji_ssc = await Interaction.guild.emojis.cache.find(emoji => emoji.name === 'EMOJIS7')
