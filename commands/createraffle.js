...
module.exports.run = async (bot, Interaction, options) =>
{
    ...
    var CongratsWinner = bot.channels.cache.find(channel => channel.id === `1002606816473141288`);
    if(Interaction.channel.id != "1002606816473141288") return Interaction.editReply({content: `You can only use this command in ${CongratsWinner}`})
    ...
    if (set_description.length > 750)
    {
        return Interaction.editReply(
        {
            content: "The description is too long please enter a smaller description less than 750 characters so that the embed can be issued.", ephemeral: true//embeds: [ raffleEmbed ], components: [row], ephemeral: false
        })
    }
